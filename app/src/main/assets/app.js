...
function openTerminal() { openPanel('Terminal'); command.placeholder = 'Enter a shell command…'; command.value = ''; commandList.innerHTML = '<div class="terminal-note">Commands execute only through the native bridge and inherit the app UID boundary.</div>'; const run = document.createElement('button'); run.textContent = 'Run command'; run.onclick = runTerminalCommand; commandList.appendChild(run); command.onkeydown = (event) => { if (event.key === 'Enter') runTerminalCommand(); }; previewBody.classList.add('hidden'); command.focus(); }
