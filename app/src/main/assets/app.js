...
function activate(id) { if (!state.files.has(id)) return; const previousId = state.activeId; const current = state.files.get(previousId); if (current && previousId !== id) current.content = editor.value; state.activeId = id; const file = state.files.get(id); editor.value = file.content; workspace.textContent = 'Local workspace'; language.textContent = file.language; dirty.textContent = file.dirty ? 'Modified' : 'Saved'; renderTabs(); sync(); }
...
function previewHtml() { const file = state.files.get(state.activeId); if (!file || (!/html/i.test(file.language) && !/\.html?$/i.test(file.name))) { showTool('Preview', 'Open an HTML file to use the local preview panel.'); return; } openPanel('Preview'); hideCommandArea(); previewBody.classList.remove('hidden'); previewBody.innerHTML = ''; const frame = document.createElement('iframe'); frame.className = 'preview-frame'; frame.setAttribute('sandbox', 'allow-scripts allow-forms allow-modals'); frame.srcdoc = window.VSACDevTools ? window.VSACDevTools.instrumentHtml(file.content) : file.content; state.previewFrame = frame; previewBody.appendChild(frame); }
function openTerminal() { openPanel('Terminal'); command.placeholder = 'Enter a shell command…'; command.value = ''; command.oninput = null; commandList.innerHTML = '<div class="terminal-note">Commands execute only through the native bridge and inherit the app UID boundary.</div>'; const run = document.createElement('button'); run.textContent = 'Run command'; run.onclick = runTerminalCommand; commandList.appendChild(run); command.onkeydown = (event) => { if (event.key === 'Enter') runTerminalCommand(); }; previewBody.classList.add('hidden'); command.focus(); }
...
function openPanel(title = 'Command Palette') { state.panelMode = title; panelTitle.textContent = title; panel.classList.remove('hidden'); command.classList.remove('hidden'); commandList.classList.remove('hidden'); previewBody.classList.add('hidden'); command.placeholder = 'Type a command…'; command.oninput = renderCommands; renderCommands(); command.focus(); }
function closePanel() { panel.classList.add('hidden'); command.value = ''; command.oninput = null; state.panelMode = 'commands'; }
function renderCommands() { const query = command.value.trim().toLowerCase(); commandList.innerHTML = ''; for (const [label, action] of COMMANDS) { if (query && !label.toLowerCase().includes(query)) continue; const button = document.createElement('button'); button.textContent = label; button.onclick = action; commandList.appendChild(button); } }
